...
def load_classification_model():
    ...
    try:
        ...
        st.error(f"Error loading Classification Model. Please ensure the model file '{CLASSIFICATION_MODEL_PATH}' is in the same directory as app.py.")
        st.exception(e)
        return None
...
st.sidebar.markdown("1. Ensure **`tensorflow`**, **`streamlit`**, **`pillow`**, and **`numpy`** are installed.")
st.sidebar.markdown("2. Place `app.py` and the winning model file (`best_mobilenet_model.keras`) in the same folder.")
